...
| Phase-2 contrastive temporal variants (LSTM and Transformer backbones) | 🔄 Basic model, training, diagnostics, and frozen-feature extraction setup implemented; no full-data pretraining or downstream evaluation yet |
...
| Phase 2 decoder refinement | 🔄 D0–D4 Stage-1 pipeline implemented and real temporal/price artifacts validated; 30-run CUDA matrix frozen but not executed |
| Phase 2 encoder refinement | 🔄 Matched contrastive LSTM/Transformer model, training, diagnostics, and frozen-feature extraction setup are implemented; full-data pretraining and downstream evaluation remain pending |
...
The five-branch Phase-1 task matrix is complete for price, trend, and volatility, with reports and plots saved for all three. Phase 2 has three independent parts: decoder refinement, encoder refinement, and classification relabelling. The D0--D4 Stage-1 decoder pipeline is implemented at `K=8`, seeds `0/1/2`, and budgets `15/50/100`; its contract-safe temporal index and price labels pass real-data replay, and its 30-run CUDA matrix is frozen but unexecuted. The encoder-refinement scaffold provides matched contrastive LSTM/Transformer models, training diagnostics, and frozen-feature extraction, but its final experiment specification, full-data CUDA pretraining, and downstream matrix remain pending. The Part 3 classification specification is frozen at `h=2`, `tau=0.005`, seeds `0/1/2`, and budgets `15/50/100`; its only learned trajectories are the strict TA-aligned C1 Raw-OHLCV MLP, C2 five-branch framework, and C5 adapted TA-MLP under P0/P1U/P1O/P2. Decoder and encoder variants are excluded from the Part 3 launcher but remain the subjects of Parts 1 and 2. Full-row-only runs, representation ablations, C3, and C4 are not required by the current Phase 2 plan. The earlier launcher was stopped after this scope correction; completed intended C1/C2/C5 artifacts remain available for a clean resume. Other priorities remain executing and reporting the decoder matrix, completing the classification matrix, the Raw-OHLCV MLP volatility migration, a nonnegative framework volatility decoder, and remaining external-baseline alignment. Train-only raw-OHLCV Alpha101-style and bounded-GP dry runs demonstrate timestamped factor scoring and evolutionary formula construction without touching the global task-test partition; downstream-head OOF mining and fresh-holdout alpha evaluation remain outside this budget. Current decoder results support implementation and replay claims only because performance has not yet been measured; current framework evidence does not support universal superiority over task-specific baselines.
