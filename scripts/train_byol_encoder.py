...
class BYOLConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.batch_size <= 1:
            raise ValueError("batch_size must be greater than 1 for BYOL diagnostics")
        ...
        if not 0.0 <= self.target_decay <= 1.0:
...
def make_loader(train_sequences: np.ndarray, config: BYOLConfig) -> DataLoader:
...
def _collapse_warning(embedding_std: float, threshold: float) -> bool:
    return embedding_std < threshold


def _write_budget_summary(path: Path, metrics: dict) -> None:
    lines = [
        f"# BYOL encoder epoch {metrics['epoch']}",
        "",
        ...
        f"- target decay: `{metrics['training_config']['target_decay']}`",
        f"- final train BYOL loss: `{metrics['train_loss']:.10f}`",
        f"- final view cosine: `{metrics['view_cosine']:.10f}`",
        f"- final embedding std: `{metrics['embedding_std']:.10f}`",
        f"- final embedding norm: `{metrics['embedding_norm']:.10f}`",
        f"- collapse warning: `{metrics['collapse_warning']}`",
        f"- best observed train BYOL loss: `{metrics['best_train_loss']:.10f}`",
...
def train_and_snapshot(
    ...
    for epoch in range(1, max(config.epoch_budgets) + 1):
        ...
        print(
            f"byol epoch completed: epoch={epoch} loss={losses['loss']:.8f} "
            f"view_cosine={losses['view_cosine']:.8f} "
            f"embedding_std={losses['embedding_std']:.8f} "
        ...
        collapse_warning = _collapse_warning(losses["embedding_std"], collapse_std_threshold)
        metrics = {
            "model": "byol_encoder",
            "epoch": epoch,
            "seed": config.seed,
            ...
            "view_cosine": float(losses["view_cosine"]),
            "embedding_std": float(losses["embedding_std"]),
            "embedding_norm": float(losses["embedding_norm"]),
        ...
        print(f"byol checkpoint saved: epoch={epoch} path={checkpoint_path}", flush=True)
        ...
        if canonical_checkpoint is not None and epoch == max(config.epoch_budgets):
            ...
            print(f"canonical byol checkpoint saved: path={canonical_checkpoint}", flush=True)
...
def run_experiment(
    ...
    _json_write(
        run_root / "dataset_manifest.json",
        {
            ...
            "test_usage": "not used for BYOL pretraining",
        },
    )
    return train_and_snapshot(
        train_sequences,
        processed_npz,
        run_root,
        config,
...
def _default_run_root(run_name: str) -> Path:
    ...
    return ROOT / "experiments" / "byol_encoder" / run_name
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Pretrain the BYOL encoder on the locked train split.")
    parser.add_argument("--processed-npz", default="data/processed/market_4h_seq64_top50.npz")
    parser.add_argument("--run-name", default="byol-4h-seq64-top50")
    ...
    parser.add_argument("--target-decay", type=float, default=0.99)
    parser.add_argument("--collapse-std-threshold", type=float, default=1e-3)
    parser.add_argument("--device", default="auto")
    parser.add_argument(
        "--canonical-checkpoint",
        default=None,
        help="Optional path for a copy of the final budget checkpoint, e.g. checkpoints/byol_4h_seq64_top50.pth.",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(
            "byol encoder experiment completed: "
        ...
        print(f"byol encoder experiment failed: {exc}", file=sys.stderr)
        return 1
